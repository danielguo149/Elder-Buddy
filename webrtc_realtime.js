 ...
 */
console.log('webrtc_realtime.js file is loading...');
window.testVariable = 'Script loaded successfully';
...
console.log('globalIsSessionActive:', window.globalIsSessionActive);
console.log('globalDataChannel:', window.globalDataChannel);
console.log('dataChannel readyState:', window.globalDataChannel?.readyState);

// Create window.realtime early, outside the DOMContentLoaded
window.realtime = {
    isActive() {
        return !!(isSessionActive && dataChannel && dataChannel.readyState === "open");
    },
    ...
    start() { 
        if (typeof startSession === 'function') {
            ...
            console.error('startSession function not available yet');
        }
    },
    ...
    stop() { 
        if (typeof stopSession === 'function') {
            ...
            console.error('stopSession function not available yet');
        }
    },
  ...
  sendText(text) {
    if (this.isActive()) {
        try {
            // Create a conversation item (text message from user)
            const createMessage = {
                event_id: `event_${Date.now()}_create`,
            ...
            // Add to transcript if the function exists
            if (typeof addTranscriptMessage === 'function') {
                addTranscriptMessage(text, "user");
            }
            
            // Show waiting indicator if it exists
            if (typeof showWaitingIndicator === 'function') {
        ...
        // Queue the message and try to start
        console.log('Realtime not active, queuing message:', text);
        this._queuedMessages = this._queuedMessages || [];
        this._queuedMessages.push(String(text));
    ...
    // Internal queued messages
    _queuedMessages: [],
    
    // Flush queued messages when connection becomes active
    _flushQueued() {
        if (this.isActive() && this._queuedMessages.length > 0) {
            console.log('Flushing', this._queuedMessages.length, 'queued messages');
...
console.log('window.realtime created:', !!window.realtime);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // === Chess glue & adapter (paste inside DOMContentLoaded, before the final "});") ===
    const _pendingTexts = [];

    // Flush any queued text once we're active
    function _flushPending() {
    if (!window.realtime.isActive()) return;
    while (_pendingTexts.length) {
        const t = _pendingTexts.shift();
        try {
        dataChannel.send(JSON.stringify({ type: "input_text", text: t }));
        addTranscriptMessage(t, "user");
        } catch (e) {
        debugLog("flush send error: " + e.message, "error");
        break;
        }
    }
    showWaitingIndicator(false);
